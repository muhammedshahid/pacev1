...
Let \( L = \{L_i\}_{i=1}^{N} \) denote the luminance values.
...
$$
\sigma^2 = \max\left(0,\; \frac{1}{N} \sum_{i=1}^{N} L_i^2 - \mu^2 \right) \tag{2}
$$
...
$$
\text{Skewness} = \frac{1}{N (\sigma + \epsilon)^3} \sum_{i=1}^{N} (L_i - \mu)^3 \tag{4}
$$

$$
\text{Kurtosis} = \frac{1}{N (\sigma + \epsilon)^4} \sum_{i=1}^{N} (L_i - \mu)^4 \tag{5}
$$

$$
H = - \sum_{k} p_k \log_2(p_k + \epsilon) \tag{6}
$$
...
$$
R_s = \frac{1}{N} \sum_{i=1}^{N} \mathbf{1}(L_i < 0.2) \tag{9}
$$

$$
R_h = \frac{1}{N} \sum_{i=1}^{N} \mathbf{1}(L_i > 0.8) \tag{10}
$$
...
$$
\alpha(x,y) = \mathrm{clip}\Big(\alpha \big[1 + 1.2(C_{\text{loc}} - 0.5)\big], 0.05, 2\alpha\Big) \tag{17}
$$
